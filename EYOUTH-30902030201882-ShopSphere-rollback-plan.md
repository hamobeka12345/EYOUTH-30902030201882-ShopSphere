...
UptimeRobot monitors `https://eyouth-30902030201882-shopsphere-ba.vercel.app/api/health` every few minutes. A non-200 response, timeout, or missing `status: "ok"` means the release failed. UptimeRobot sends alerts via email, SMS, or webhook when the endpoint goes down.
...
6. Verify `https://eyouth-30902030201882-shopsphere-ba.vercel.app/api/health` returns `status: "ok"`.
